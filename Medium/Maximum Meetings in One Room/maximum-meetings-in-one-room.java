...
class Solution {
    ...
    public static ArrayList<Integer> maxMeetings(int n, int[] start, int[] end) {
        ...
        for (int i = 0; i < n; i++) {
            meetings.add(new Pair(start[i], end[i], i));
        }
      
        Collections.sort(meetings, (a, b) -> a.end - b.end);
        
        ArrayList<Integer> result = new ArrayList<>();
        result.add(meetings.get(0).index + 1);  // Adding the index of the first meeting
        
        int prevEnd = meetings.get(0).end;
        for (int i = 1; i < n; i++) {
            if (meetings.get(i).start > prevEnd) {
                result.add(meetings.get(i).index + 1);
                prevEnd = meetings.get(i).end;
            }
        }
        
        Collections.sort(result); // Sort the result to maintain original indices
